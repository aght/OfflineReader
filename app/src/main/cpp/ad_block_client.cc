        ...
        if (parseState != FPDataOnly)
        {
            ...
            case '|':
                ...
                else if (parseState == FPOneBar)
                {
                    ...
                    if ((*(p + len) == '^' && (*(p + len + 1) == '\0' || *(p + len + 1) == '$' || isEndOfLine(*(p + len + 1)))) ||
                        *(p + len) == '\0' || *(p + len) == '$' ||
                        isEndOfLine(*(p + len)))
                    {
                        f->filterType =
...
void AdBlockClient::clear()
{
    ...
    // NOTE: Added to make the object cleanup the passed buffer itself
                            ...
                            const char *contextDomain)
{
    ...
    if (!hasMatch)
    {
        bloomFilterMiss = bloomFilter && !bloomFilter->substringExists(input, AdBlockClient::kFingerprintSize);
    ...
    bool bloomExceptionFilterMiss = exceptionBloomFilter && !exceptionBloomFilter->substringExists(input,
...
bool AdBlockClient::parse(const char *input, bool preserveRules)
{
    ...
    Filter *newNoFingerprintExceptionFilters =
        new Filter[newNumNoFingerprintExceptionFilters + numNoFingerprintExceptionFilters];
    ...
    Filter *newNoFingerprintDomainOnlyExceptionFilters =
        new Filter[newNumNoFingerprintDomainOnlyExceptionFilters + numNoFingerprintDomainOnlyExceptionFilters];
    Filter *newNoFingerprintAntiDomainOnlyExceptionFilters =
        new Filter[newNumNoFingerprintAntiDomainOnlyExceptionFilters + numNoFingerprintAntiDomainOnlyExceptionFilters];
    ...
    memset(newNoFingerprintExceptionFilters, 0,
           sizeof(Filter) * (newNumNoFingerprintExceptionFilters + numNoFingerprintExceptionFilters));
    ...
    memset(newNoFingerprintDomainOnlyExceptionFilters, 0,
           sizeof(Filter) * (newNumNoFingerprintDomainOnlyExceptionFilters + numNoFingerprintDomainOnlyExceptionFilters));
    memset(newNoFingerprintAntiDomainOnlyExceptionFilters, 0,
           sizeof(Filter) * (newNumNoFingerprintAntiDomainOnlyExceptionFilters + numNoFingerprintAntiDomainOnlyExceptionFilters));
                               ...
                               bool ignoreHtmlFilters)
{
    ...
    *totalSize += 1 + snprintf(sz, sizeof(sz),
                               ...
                               bloomFilter ? bloomFilter->getByteBufferSize() : 0, exceptionBloomFilter ? exceptionBloomFilter->getByteBufferSize() : 0,
